...
if(WIZARD_INSTALL_DEMO_DATA){
	if(!$iblockID){
		...
		if ($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"]) {
			$ID_SKU = CCatalog::LinkSKUIBlock($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"], $iblockID);
			$rsCatalogs = CCatalog::GetList(array(), array('IBLOCK_ID' => $iblockID), false, false, array('IBLOCK_ID'));
			if($arCatalog = $rsCatalogs->Fetch()){
				CCatalog::Update($iblockID, array('PRODUCT_IBLOCK_ID' => $_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"], 'SKU_PROPERTY_ID' => $ID_SKU));
			}
			else{
				CCatalog::Add(array('IBLOCK_ID' => $iblockID, 'PRODUCT_IBLOCK_ID' => $_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"], 'SKU_PROPERTY_ID' => $ID_SKU));
			}		
		}
	}
	else{
	...
	if ($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"]) {
		//create facet index
		$index = \Bitrix\Iblock\PropertyIndex\Manager::createIndexer($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"]);
		$index->startIndex();
	...
	if($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"] && method_exists('\Bitrix\Iblock\ElementTable', 'getCount'))
	{
		$count = \Bitrix\Iblock\ElementTable::getCount(array(
			'=IBLOCK_ID' => $_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"],
			'=WF_PARENT_ELEMENT_ID' => null
		));
		if ($count > 0)
		{
			$catalogReindex = new CCatalogProductAvailable('', 0, 0);
			$catalogReindex->initStep($count, 0, 0);
			$catalogReindex->setParams(array('IBLOCK_ID' => $_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"]));
			$catalogReindex->run();
			unset($catalogReindex);
		}
		unset($_SESSION["WIZARD_NEXT_CATALOG_IBLOCK_ID"]);
	}
